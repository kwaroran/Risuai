export const languageChinese = {
    ...
    webuiApiFlagWarning: "必须使用带有 --api 参数的 WebUI",
    webuiAgplWarning: "如需查看 AGPL 许可证内容，必须使用不含 AGPL 许可证的 WebUI，或在使用 AGPL 许可证时使用未经修改的版本。",
    webVersionTunnelWarning: "你正在使用网页版。要使用本地 WebUI，必须使用 ngrok 等内网穿透工具。",
    comfyCorsHeaderWarning: "\"请使用 --enable-cors-header 参数运行 comfyUI。\"",
    comfyFirstImageNote: "将选用提示词生成的第一张图片。",
    leaveBlankForDefaultImage: "留空以使用角色的默认图片。",
    useCharacterDefaultImage: "使用角色默认图片。",
    modelImageInputUnsupported: "该模型不支持图片输入，或刷新模型列表以更新模型状态。",
    selectAModel: "选择模型...",
    notAllowed: "无权访问",
    ttsAutoImportWarning: "如果不设置为 Auto，从其他操作系统或浏览器导入时可能无法正常工作。",
    elevenLabsKeyLocationHint: "请在\"全局设置 → 机器人设置 → 其他 → ElevenLabs API 密钥\"中设置 ElevenLabs API 密钥",
    modelFetchError: "获取模型时发生错误。",
    realmLoginRequired: "上传到 RisuRealm 需要登录 Risu 账户",
    realmLoginHint: "可在应用设置 🡲 账户中登录",
    realmCreatorNotesDesc: "在搜索和首次打开机器人时显示的说明。",
    realmCreatorNotesLengthHint: "至少 20 个字符。",
    realmTagsDesc: "用于搜索角色的标签。仅支持拉丁字母，用逗号分隔。",
    realmLicenseDesc: "你可以选择许可证来限制下载者对卡片提示词的使用。",
    welcomeToRisuai: "欢迎使用 Risuai！",
    selectBotToStartChatting: "选择一个机器人开始聊天",
    tokenEstimateDisclaimer: "这是估算值，实际 token 数量可能有所不同。",
    irisTypingIndicator: "Iris 正在输入",
    promptDiffCardsChanged: "变更的卡片：",
    promptDiffNoBodyContent: "无正文内容",
    promptDiffNotComputedYet: "尚未计算差异。",
    promptDiffNoChanges: "无变更",

export const languageChineseTraditional = {
    ...
    webuiApiFlagWarning: "必須使用帶有 --api 參數的 WebUI",
    webuiAgplWarning: "如需檢視 AGPL 授權條款內容，必須使用不含 AGPL 授權條款的 WebUI，或在使用 AGPL 授權條款時使用未經修改的版本。",
    webVersionTunnelWarning: "你正在使用網頁版。若要使用本機 WebUI，必須使用 ngrok 等通道工具。",
    comfyCorsHeaderWarning: "\"請使用 --enable-cors-header 參數執行 comfyUI。\"",
    comfyFirstImageNote: "將選用提示詞生成的第一張圖片。",
    leaveBlankForDefaultImage: "留空以使用角色的預設圖片。",
    useCharacterDefaultImage: "使用角色預設圖片。",
    modelImageInputUnsupported: "該模型不支援圖片輸入，或重新整理模型列表以更新模型狀態。",
    selectAModel: "選擇模型...",
    notAllowed: "無權存取",
    ttsAutoImportWarning: "如果未設定為 Auto，從其他作業系統或瀏覽器匯入時可能無法正常運作。",
    elevenLabsKeyLocationHint: "請在\"全域設定 → 機器人設定 → 其他 → ElevenLabs API 金鑰\"中設定 ElevenLabs API 金鑰",
    modelFetchError: "取得模型時發生錯誤。",
    realmLoginRequired: "上傳到 RisuRealm 需要登入 Risu 帳號",
    realmLoginHint: "可在應用程式設定 🡲 帳號中登入",
    realmCreatorNotesDesc: "在搜尋和首次開啟機器人時顯示的說明。",
    realmCreatorNotesLengthHint: "至少 20 個字元。",
    realmTagsDesc: "用於搜尋角色的標籤。僅支援拉丁字母，以逗號分隔。",
    realmLicenseDesc: "你可以選擇授權條款來限制下載者對卡片提示詞的使用。",
    welcomeToRisuai: "歡迎使用 Risuai！",
    selectBotToStartChatting: "選擇一個機器人開始對話",
    tokenEstimateDisclaimer: "這是估算值，實際 token 數量可能有所不同。",
    irisTypingIndicator: "Iris 正在輸入",
    promptDiffCardsChanged: "變更的卡片：",
    promptDiffNoBodyContent: "無本文內容",
    promptDiffNotComputedYet: "尚未計算差異。",
    promptDiffNoChanges: "無變更",

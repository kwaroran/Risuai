...
{#if submenu === 3 || submenu === -1}
    <Accordion name={language.imageGeneration} styled disabled={submenu !== -1}>
        ...
        <span class="text-draculared text-xs mb-2">{language.webuiApiFlagWarning}</span>
            <span class="text-draculared text-xs mb-2">{language.webuiAgplWarning}</span>
            {#if !isTauri}
                <span class="text-draculared text-xs mb-2">{language.webVersionTunnelWarning}</span>
            {/if}
        ...
        {#if DBState.db.sdProvider === 'novelai'}
            ...
            {#if DBState.db.NAIImgConfig.reference_mode === 'character' && 
                ...
                <span class="text-textcolor2 text-xs mb-2 block">{language.leaveBlankForDefaultImage}</span>
            ...
            {#if DBState.db.NAII2I}
                ...
                <span class="text-textcolor2 text-xs block">{language.leaveBlankForDefaultImage}</span>
        ...
        {#if DBState.db.sdProvider === 'comfy'}
            <span class="text-draculared text-xs mb-2">{language.comfyFirstImageNote}</span>
            {#if !isTauri}
                <span class="text-draculared text-xs mb-2">{language.comfyCorsHeaderWarning}</span>
            {/if}
            <span class="text-textcolor mt-2">ComfyUI {language.providerURL}</span>
            <TextInput size="sm" marginBottom placeholder="http://127.0.0.1:8188" bind:value={DBState.db.comfyUiUrl}/>
        ...
        {#if DBState.db.sdProvider === 'wavespeed'}
            ...
            <SelectInput className="mb-4" bind:value={DBState.db.wavespeedImage.model} onchange={handleModelChange}>
                <OptionInput value="" >{language.selectAModel}</OptionInput>
            ...
            {#if wavespeedModels.find(m => m.model_id === DBState.db.wavespeedImage.model)?.supportsImageInput}
                ...
                {#if DBState.db.wavespeedImage.reference_mode === 'character'}
                    <span class="text-textcolor2 text-xs mb-2 block">{language.useCharacterDefaultImage}</span>
                {/if}
            {:else}
                <span class="text-textcolor2 text-xs mb-2 block">
                    {language.modelImageInputUnsupported}
                </span>
            {/if}
        {/if}
    </Accordion>
